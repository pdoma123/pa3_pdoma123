...
bool NeuralNetwork::update() {
	...
	nodes[i]->delta = 0.0;
        ...
        for (pair<const int, Connection>& c : adjacencyList[i]) {
            
	    c.second.weight -= (learningRate * c.second.delta);
            c.second.delta = 0.0;

	    if (find(visited.begin(), visited.end(), c.first) == visited.end()) {
            	q.push(c.first);
		visited.push_back(c.first);
	    }
        }
    }
